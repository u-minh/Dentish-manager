...
BEGIN TRY
	...
	ELSE
		BEGIN
			...
			INSERT INTO TAI_KHOAN (TenTK, MatKhau, LoaiTK) 
...
BEGIN TRY
	BEGIN TRAN
	SET TRANSACTION ISOLATION LEVEL READ COMMITTED; -- mặc định của sql server

	IF EXISTS (SELECT * FROM LICH_HEN WHERE @date = NgayKham and @time = GioKham and @ID_NS = ID_NS)
		BEGIN
			ROLLBACK TRAN;
...
BEGIN TRY
	BEGIN TRAN
		...
		INSERT INTO BENH_AN (NgayKham, ID_KH, ID_NS, DichVu, Thuoc)
...
BEGIN TRY
	BEGIN TRAN
		SET TRANSACTION ISOLATION LEVEL READ COMMITTED;
		IF NOT EXISTS (SELECT * FROM LICH_NHA_SI LNS WHERE LNS.ID_NS = @ID_NS and NGAY_KHAM = @NgayKhamCu and GIO_KHAM = @GioKhamCu)
		BEGIN
			ROLLBACK TRAN;
			THROW 50001, 'Dentish do not have this schedule, can not edit!', 1;
		END
		IF EXISTS (SELECT * FROM LICH_HEN WHERE ID_NS = @ID_NS and NgayKham = @NgayKham and GioKham = @GioKham)
		BEGIN
			ROLLBACK TRAN;
			THROW 50001, 'Dentish already have this schedule as an appointment with customer, can not add an edit!', 1;
		END

		WAITFOR DELAY '00:00:05' -- chờ khóa

		UPDATE LICH_NHA_SI --WITH (XLOCK) 
...
BEGIN TRY
	BEGIN TRAN
		...
		ELSE
		BEGIN
			INSERT INTO LICH_NHA_SI (ID_NS, NGAY_KHAM, GIO_KHAM)
...
BEGIN
    BEGIN TRY
		...
		BEGIN
			ROLLBACK TRAN;
			THROW 50001, 'You manage have at least one staff.', 1;
		END
